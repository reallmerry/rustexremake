...
implements InputStreamStatistics {
    ...
    protected final int readBackReference(byte[] b2, int off, int len) {
        int avail = this.available();
        if (len > avail) {
            this.tryToCopy(len - avail);
        }
        return this.readFromBuffer(b2, off, len);
    }

    private int readFromBuffer(byte[] b2, int off, int len) {
        int readable = Math.min(len, this.available());
        if (readable > 0) {
            System.arraycopy(this.buf, this.readIndex, b2, off, readable);
    ...
    protected final int readLiteral(byte[] b2, int off, int len) throws IOException {
        int avail = this.available();
        if (len > avail) {
