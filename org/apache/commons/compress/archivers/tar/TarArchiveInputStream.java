...
extends ArchiveInputStream {
    ...
    private long skipSparse(long n2) throws IOException {
        if (this.sparseInputStreams == null || this.sparseInputStreams.isEmpty()) {
            return this.inputStream.skip(n2);
        }
        long bytesSkipped = 0L;
        while (bytesSkipped < n2 && this.currentSparseInputStreamIndex < this.sparseInputStreams.size()) {
            InputStream currentInputStream = this.sparseInputStreams.get(this.currentSparseInputStreamIndex);
            if ((bytesSkipped += currentInputStream.skip(n2 - bytesSkipped)) >= n2) continue;
            ++this.currentSparseInputStreamIndex;
        }
        return bytesSkipped;
    }
