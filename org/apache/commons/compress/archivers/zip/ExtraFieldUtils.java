...
public class ExtraFieldUtils {
    ...
    public static ZipExtraField[] parse(byte[] data, boolean local, ExtraFieldParsingBehavior parsingBehavior) throws ZipException {
        ...
        for (int start = 0; start <= dataLength - 4; start += length + 4) {
            ...
            try {
                ...
                v2.add(Objects.requireNonNull(parsingBehavior.fill(ze, data, start + 4, length, local), "fill must not return null"));
                continue;
            } catch (IllegalAccessException | InstantiationException ie) {
                throw (ZipException)new ZipException(ie.getMessage()).initCause(ie);
            }
        }
        return v2.toArray(EMPTY_ZIP_EXTRA_FIELD_ARRAY);
    }
    ...
    public static void register(Class<?> c2) {
        try {
            ...
            throw new IllegalArgumentException(c2 + " doesn't implement ZipExtraField");
        } catch (InstantiationException ie) {
            throw new IllegalArgumentException(c2 + " is not a concrete class");
        } catch (IllegalAccessException ie) {
            throw new IllegalArgumentException(c2 + "'s no-arg constructor is not public");
        }
    }
