...
implements Closeable {
    ...
    public ZipFile(File f2, String encoding) throws IOException {
        this(f2.toPath(), encoding, true);
    }

    public ZipFile(File f2, String encoding, boolean useUnicodeExtraFields) throws IOException {
        this(f2.toPath(), encoding, useUnicodeExtraFields, false);
    }

    public ZipFile(File f2, String encoding, boolean useUnicodeExtraFields, boolean ignoreLocalFileHeader) throws IOException {
        this(Files.newByteChannel(f2.toPath(), EnumSet.of(StandardOpenOption.READ), new FileAttribute[0]), f2.getAbsolutePath(), encoding, useUnicodeExtraFields, true, ignoreLocalFileHeader);
    }
    ...
    private ZipFile(SeekableByteChannel channel, String archiveName, String encoding, boolean useUnicodeExtraFields, boolean closeOnError, boolean ignoreLocalFileHeader) throws IOException {
        ...
        try {
            ...
            try {
                throw new IOException("Error on ZipFile " + archiveName, e2);
    ...
    private void fillNameMap() {
        this.entries.forEach(ze -> {
            ...
            LinkedList entriesOfThatName = this.nameMap.computeIfAbsent(name, k2 -> new LinkedList());
    ...
    public InputStream getInputStream(ZipArchiveEntry zipEntry) throws IOException {
        ...
        BufferedInputStream is = new BufferedInputStream(this.getRawInputStream(zipEntry));
        switch (ZipMethod.getMethodByCode(zipEntry.getMethod())) {
            ...
            case IMPLODING: {
                try {
                    return new ExplodingInputStream(zipEntry.getGeneralPurposeBit().getSlidingDictionarySize(), zipEntry.getGeneralPurposeBit().getNumberOfShannonFanoTrees(), is);
    ...
    private void readCentralDirectoryEntry(Map<ZipArchiveEntry, NameAndComment> noUTF8Flag) throws IOException {
        ...
        try {
            ...
            ZipException z2 = new ZipException("Invalid extra data in entry " + ze.getName());
            z2.initCause(ex);
            throw z2;
        }
