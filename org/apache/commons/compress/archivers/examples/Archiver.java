...
public class Archiver {
    ...
    public void create(String format, OutputStream target, File directory, CloseableConsumer closeableConsumer) throws IOException, ArchiveException {
        try (CloseableConsumerAdapter c2 = new CloseableConsumerAdapter(closeableConsumer);){
            this.create(c2.track(ArchiveStreamFactory.DEFAULT.createArchiveOutputStream(format, target)), directory);
        }
    }
    ...
    public void create(String format, SeekableByteChannel target, File directory, CloseableConsumer closeableConsumer) throws IOException, ArchiveException {
        block8: {
            try (CloseableConsumerAdapter c2 = new CloseableConsumerAdapter(closeableConsumer);){
                if (!this.prefersSeekableByteChannel(format)) {
                    this.create(format, c2.track(Channels.newOutputStream(target)), directory);
                    break block8;
                }
                if ("zip".equalsIgnoreCase(format)) {
                    this.create((ArchiveOutputStream)c2.track(new ZipArchiveOutputStream(target)), directory);
                    break block8;
                }
                if ("7z".equalsIgnoreCase(format)) {
                    this.create(c2.track(new SevenZOutputFile(target)), directory);
                    break block8;
                }
